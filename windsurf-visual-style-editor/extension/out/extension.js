...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('visualStyleEditor.properties', provider), vscode.commands.registerCommand('visualStyleEditor.startBridge', startBridge), vscode.commands.registerCommand('visualStyleEditor.stopBridge', stopBridge));
    void startBridge();
}
...
async function startBridge() {
    ...
    const port = vscode.workspace.getConfiguration('visualStyleEditor').get('port', 49152);
    server = new ws_1.WebSocketServer({ host: '127.0.0.1', port });
        ...
        socket.on('message', raw => {
            try {
                ...
                if (message.type === 'selected')
                    provider.setSelection(message.element);
            }
            catch (error) {
                console.error(error);
            }
...
class PropertiesProvider {
    ...
    resolveWebviewView(view) {
        ...
        view.webview.onDidReceiveMessage(async (message) => {
            ...
            if (message.type === 'pick')
                this.sendBrowser({ type: 'pick' });
            if (message.type === 'change' && this.selection)
                this.queueChange(message.property, message.value);
            if (message.type === 'send')
                await this.sendToAgent(message.description);
            if (message.type === 'discard')
                this.discardPending();
        });
    }
    setConnection(connected) { this.connected = connected; this.pushState(); }
    ...
    sendBrowser(message) {
        const open = [...browserSockets].filter(socket => socket.readyState === ws_1.WebSocket.OPEN);
        if (!open.length) {
...
function buildAgentPrompt(edit, description) {
...
function getHtml() {
    const colorFields = [['color', 'Text color'], ['background-color', 'Background'], ['border-color', 'Border color']];
    const textFields = [
    ...
    const sides = ['T', 'R', 'B', 'L'];
    const box = (prop, label) => `<div class="field"><span>${label}</span><span class="box4">${sides.map(s => `<input data-box="${prop}" placeholder="${s}" />`).join('')}</span></div>`;
